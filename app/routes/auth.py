...
async def registration(
    ...
    result = await uow.users.get(email=request.email)
    if result:
    ...
    await uow.users.update({"access_token": secret}, id=user.id)
    await uow.commit()
...
async def login(request: AuthRequest, uow: SQLAlchemyUnitOfWork = Depends(get_uow)):
    user: User | None = await uow.users.get(email=request.email)
